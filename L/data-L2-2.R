...
dat_L2.load <- function(geno_table=L_marker_st$marker,pheno_table="L-2.txt" ){
  dat_L2<-list(TIME=NULL,
               geno=NULL,
               pheno=NULL,
               trait=1)
  ...
  B_F<-read.table("L-2.txt",header=TRUE)
  B_F<-as.matrix(B_F)
  A1_F<-A_F[,1]
  B1_F<-B_F[,1]
  ...
  dat_L2$geno <- AA1_F.1
  dat_L2$pheno <- BB1_F.1
  return(dat_L2)
}
dat_L2_st <- dat_L2.load(geno_table=L_marker_st$marker,pheno_table="L-2.txt")
...
LR_pistils <-LR_mul_value(1,dat_L2_st$geno,dat_L2_st$pheno,2)
LR_petals <-LR_mul_value(3,dat_L2_st$geno,dat_L2_st$pheno,2)
...
LR_pistils_permu2<-LR_mul_permu(1,dat_L2_st$geno,dat_L2_st$pheno,geno_type_st$marker2,2)#
LR_pistils_permu3<-LR_mul_permu(1,dat_L2_st$geno,dat_L2_st$pheno,geno_type_st$marker3,2)#

plot(LR_petals[geno_type_st$marker2])
plot(LR_petals[geno_type_st$marker3])
LR_petals_permu2<-LR_mul_permu(3,dat_L2_st$geno,dat_L2_st$pheno,geno_type_st$marker2,2)#
LR_petals_permu3<-LR_mul_permu(3,dat_L2_st$geno,dat_L2_st$pheno,geno_type_st$marker3,2)# 

#AA<-LR_kruskal_permu(3,dat_L2_st$geno,dat_L2_st$pheno,geno_type_st$marker2)
